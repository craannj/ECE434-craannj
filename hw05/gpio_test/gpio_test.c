...
static unsigned int gpioLED1 = 60;       ///< hard coding the LED gpio for this example to P9_23 (GPIO49)
...
static unsigned int gpioButton2 = 65;   ///< hard coding the button gpio for this example to P9_27 (GPIO115)
...
static bool	    led2On = 0;          ///< Is the LED on or off? Used to invert its state (off by default)
...
// gpio_set_value(gpioLED, ledOn);          // Not required as set by line above (here for reference)
   ...
   printk(KERN_INFO "GPIO_TEST: The button 2 state is currently: %d\n", gpio_get_value(gpioButton2));
